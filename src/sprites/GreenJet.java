...
// 3rd type of enemy, faster, stronger, and shoots faster, found in level 5
public class GreenJet extends EnemyJet {
	...
	public GreenJet(Map map, double dx, double dy, long spawnTime, int spawnLocation) {
		super(map);
...
//		damage = 15;
		...
		delay = 1500; // delay in throwing pattern
		
		try {
			// load images
			BufferedImage spritesheet = ImageIO.read(getClass().getResourceAsStream("/entities/greenJet.gif"));
	...
	public void update(double px, double py) {
		...
		if (canShoot) {
			Bullet d = new Bullet(map, 0, 4.8, "/entities/bullet.gif");
			d.setPosition(x, y);
			bullet.add(d);
			canShoot = false;
	...
	public void draw(Graphics2D g) {
		...
		// draws flashing when green jet is stunned
		if (stun) {
