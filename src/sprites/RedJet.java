...
// first basic enemy found in levels 1, 2, and 3
public class RedJet extends EnemyJet {
	...
	public RedJet(Map map, double dx, double dy, long spawnTime, int spawnLocation) {
		super(map);
		...
		canShoot = false; // default canShoot
		...
		try {
			// load images
			BufferedImage spritesheet = ImageIO.read(getClass().getResourceAsStream("/entities/redJet.gif"));
	...
	public void update(double px, double py) {
		...
		// remove bullet if it hit something and should be removed after animation
	...
	public void draw(Graphics2D g) {
		...
		// draws flashing when red jet is stunned
		if (stun) {
