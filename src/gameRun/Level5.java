...
// 5th level of the game
public class Level5 extends AbstractLevelSet {
	...
	public void init() {
		...
		jetMovementData = new double[][] { { 2.5, 0 }, { 1.2, 0 }, { -1.4, 0 },
				{ -2, 0 }, { 0, 1.2 }, { 2.9, 0 }, { 2, .0 }, { -2, .64 },
				{ -1.36, 0 }, { 2.5, 1 }, { -3, 0 }, { 2, 0 }, { 2, 0 },
				{ .4, 0 }, { -3, .8 } };
		...
		for (int i = 0; i < jetMovementData.length; i++) {
			GreenJet gjet = new GreenJet(map, jetMovementData[i][0],
					jetMovementData[i][1], (long) jetSpawnData[i][0],
					jetSpawnData[i][1]);
			gjet.setPosition(jetSpawnData[i][1], 60);
			js.add(gjet);
		}
	...
	public void update() {
		...
		// goes to GameBeat screen if question is answered - good luck getting here
		if (endLevelQuestion) {
			try {
				Thread.sleep(1000);
			} catch (Exception e) {
