...
// 1st level of the game
public class Level1 extends AbstractLevelSet {
	...
	public void init() {
		...
		jetMovementData = new double[][] { { .5, .2 }, { -.5, .2 },
				{ -.3, .9 }, { 1, 0 } };
		// { time of appearance, location of appearance }
		jetSpawnData = new int[][] { { 3000, 360 }, { 4000, 260 },
				{ 6000, 580 }, { 10000, 670 } };
		
		// { hidden life spawn time, disappear time, x position, y position }
		lifeAppearanceData = new int[] { 7000, 15000, 50, 120 };
	...
	public void update() {
		...
		p.update();
		p.checkHit(j);
		if (currentTime > lifeAppearanceData[0] && currentTime < lifeAppearanceData[1]) {
			p.checkGetLife(nl);
		}
