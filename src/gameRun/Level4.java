...
public class Level4 extends AbstractLevelSet {
	...
	public void init() {
		...
		jetMovementData = new double[][] { { 2, .2 }, { -2, .5 }, { 3, 0 },
				{ 1, 1 }, { 0, .2 }, { -1.5, -1 }, { 2, 1.5 }, { 1.98, .64 },
				{ -1.36, .32 }, { -2.5, .016 } };
		// { time of appearance, location of appearance }
		jetSpawnData = new int[][] { { 2000, 350 }, { 10000, 200 },
				{ 11000, 550 }, { 12000, 300 }, { 25000, 400 }, { 25500, 350 },
				{ 26000, 500 }, { 30000, 100 }, { 31000, 350 }, { 32000, 600 } };

		// { hidden life spawn time, disappear time, x position, y position }
		lifeAppearanceData = new int[] { 17000, 21000, 650, 400 };

		// loads layout of level
		...
		for (int i = 0; i < jetMovementData.length; i++) {
			BlueJet bjet = new BlueJet(map, jetMovementData[i][0],
					jetMovementData[i][1], (long) jetSpawnData[i][0],
					jetSpawnData[i][1]);
			bjet.setPosition(jetSpawnData[i][1], 60);
			js.add(bjet);
		}
	...
	public void update() {
		...
		// goes to level 5 if question is answered
		if (endLevelQuestion) {
			try {
				Thread.sleep(1000);
			} catch (Exception e) {
			...
			clip.stop();
			lm.setLevel(LevelManager.LEVEL5);
		}
	...
	// all the user input options here for level 2
